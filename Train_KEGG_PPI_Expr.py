...
# from Utils.HGDataset_KEGG_PPI_Expr import HGDataset
...
class Trainer:
    ...
    def save_evaluation_indicators(self, indicators):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        #     写入评价指标
        file_name = os.path.join(path, "{}_KEGG_PPI_Expr.xlsx".format(self.model_name))
    ...
    def run(self, data):
        ...
        index = index + [i*3+1 for i in np.arange(16)]
        index.sort()
        data['gene'].x[:, index] = 0


        for fold in range(self.folds):
            test_pred, test_ACC, test_F1, test_AUROC, test_AUPR = self.inference(data=data, mask=np.arange(data['gene'].y.shape[0]),
                                                                                            fold=fold)
